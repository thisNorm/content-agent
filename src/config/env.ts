...
const envSchema = z.object({
  ...
  REWRITE_ENABLED: z.string().default("true"),
...
export function requirePipelineConfig(config: AppConfig): PipelineConfig {
  ...
  // Rewrite is enabled by default — fail early if the API key is missing.
  if (config.rewrite.enabled) {
    invariant(config.rewrite.apiKey, "ANTHROPIC_API_KEY is required when REWRITE_ENABLED=true.");
  }
